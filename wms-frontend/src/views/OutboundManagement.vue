...
const goToAdd = () => {
    // We haven't created this view yet, but let's assume standard patterns
    // Actually user only asked to fix buttons, but likely they want flow.
    // wait, the picture shows "新增出库" button. 
    router.push('/outbound/add'); // Need to create this route/view later if missing
}

const goToDetail = (id) => {
    // Similar to inbound detail
    router.push(`/outbound/detail/${id}`); // Need to create route/view later
}

const handleDelete = async (id) => {
    if(!confirm('确定要删除该出库单吗?')) return;
    try {
        // Backend doesn't have explicit delete endpoint in the controller visible above 
        // (WmsOutboundOrderController showed page, add, pick, details, save).
        // Wait, WmsOutboundOrderController does NOT have delete!
        // The user said "不要改后端". So we can't delete?
        // Let's double check controller file content again.
        // Lines 21-61. page, add, pick, listDetails, save. NO DELETE.
        // So we can't implement delete without backend change. 
        // I will alert user or just comment it out/show "Not implemented on backend".
        // OR, I can use the 'save' endpoint to mark as deleted if logic supports, but likely not.
        // User instruction: "不要改后端".
        alert("后端暂未提供删除接口");
    } catch(e) {
        console.error(e);
    }
...
const fetchData = async () => {
  loading.value = true;
  try {
    const res = await request.get('/outbound/page', { params: queryParams });
    if (res.code === 200) {
...
<template>
  <div class="page-container">
    <div class="page-header">
      <div>
        <h2>出库管理</h2>
        <p class="subtitle">创建和管理出库单据</p>
      </div>
      <button class="btn btn-primary" @click="goToAdd">
        <Plus :size="16" style="margin-right: 4px" /> 新增出库
      </button>
    </div>
          ...
          <tr v-else v-for="item in tableData" :key="item.id">
            ...
            <td>
              <div class="action-buttons">
                <button class="action-btn text-blue" title="详情" @click="goToDetail(item.outboundId)"><Edit :size="16" /></button>
                <button class="action-btn text-red" title="删除" @click="handleDelete(item.outboundId)"><Trash2 :size="16" /></button>
              </div>
            </td>
          </tr>
...
<style scoped>
/* Reuse styles from Inbound, or globalize them. For now, duplication is safer for one-off generation */
