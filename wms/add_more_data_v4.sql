...
INSERT IGNORE INTO base_rack (rack_id, zone_id, warehouse_id, rack_code, rack_type, del_flag, create_time) VALUES
(11, 1, 1, 'A02', 'DOUBLE', '0', NOW()),
(12, 1, 1, 'A03', 'DOUBLE', '0', NOW());

-- 批量插入库位
INSERT IGNORE INTO base_location (rack_id, zone_id, warehouse_id, location_code, row_no, layer_no, max_weight, max_volume, status, del_flag, create_time) VALUES
-- Rack 11
...
-- 2. 新增第二个仓库 "冷链中心"
INSERT IGNORE INTO base_warehouse (warehouse_id, warehouse_code, warehouse_name, address, status, del_flag, create_time) VALUES
(2, 'WH002', '冷链中心', '上海市闵行区', 1, '0', NOW());

-- 3. 新增冷链库区
INSERT IGNORE INTO base_zone (zone_id, warehouse_id, zone_code, zone_name, zone_type, status, del_flag, create_time) VALUES
...
-- 5. 添加库存
INSERT IGNORE INTO base_product (product_id, sku_code, product_name, category_id, unit_id, weight, length, width, height, del_flag) VALUES
(1, 'SKU1001', 'iPhone 15', 1, 1, 0.2, 15, 8, 1, '0'),
(2, 'SKU2001', '澳洲牛排', 1, 1, 1.0, 20, 15, 5, '0');

-- 填充电子区 (ID 1), 刚才新建的库位
INSERT IGNORE INTO wms_inventory (warehouse_id, zone_id, location_id, product_id, batch_no, total_qty, frozen_qty, del_flag, create_time)
SELECT 1, 1, location_id, 1, 'BATCH_NEW', 10, 0, '0', NOW()
FROM base_location WHERE zone_id = 1 AND location_code LIKE 'A02%' LIMIT 5;

-- 填充生鲜区 (ID 3)
INSERT IGNORE INTO wms_inventory (warehouse_id, zone_id, location_id, product_id, batch_no, total_qty, frozen_qty, del_flag, create_time)
SELECT 2, 3, location_id, 2, 'BATCH_FOOD', 20, 0, '0', NOW()
...
SELECT z.zone_name, 
       ...
       COUNT(DISTINCT CASE WHEN i.inventory_id IS NOT NULL THEN l.location_id END) as used
FROM base_zone z
